...
export function TimelineViz({ onEventClick }) {
  ...
  return (
    <div style={{ position: "relative", paddingLeft: 28 }}>
      ...
      {TL_EVENTS.map((ev, i) => (
        <button key={ev.id} onClick={() => onEventClick(ev)} className="card-btn" style={{
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 15, fontWeight: 600, color: g.t1, marginBottom: 2 }}>{t(`popups.timeline.${ev.id}.t`)}</p>
            <p style={{ fontSize: 13, color: g.t4 }}>{t(`popups.timeline.${ev.id}.s`)}</p>
          </div>
